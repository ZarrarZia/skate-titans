...
export function GameScene({ gameState, setGameState }: GameSceneProps) {
  ...
  const RoadSegment = ({position}: {position: THREE.Vector3}) => (
    <group position={position}>
       ...
       {useMemo(() => {
            ...
            for (let i = 0; i < numLines; i++) {
                lines.push(
                    <mesh key={`l-${position.z}-${i}`} position={[-LANE_WIDTH / 2, 0.02, i * -4]}>
                        <boxGeometry args={[0.1, 0.02, 2]} />
                        <meshStandardMaterial color="yellow" />
                    </mesh>
                );
                lines.push(
                    <mesh key={`r-${position.z}-${i}`} position={[LANE_WIDTH / 2, 0.02, i * -4]}>
                        <boxGeometry args={[0.1, 0.02, 2]} />
                        <meshStandardMaterial color="yellow" />
                    </mesh>
                );
  ...
  return (
    <>
      ...
      {gameState !== 'menu' && (
        <>
            <RoadSegment position={new THREE.Vector3(0,0,0)} />
            <RoadSegment position={new THREE.Vector3(0,0,-ROAD_LENGTH)} />
        </>
      )}
